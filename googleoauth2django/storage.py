...
from googleoauth2django.helpers.dictionary_storage import Storage
...
class DjangoORMStorage(Storage):
    ...
    def locked_get(self):
        ...
        Returns:
            google.oauth2.credentials.Credentials retrieved from the Django
             ORM, associated with the ``model``, ``key_value``->``key_name``
             pair used to query for the model, and ``property_name``
             identifying the ``CredentialsProperty`` field, all of which are
             defined in the constructor for this Storage object.
